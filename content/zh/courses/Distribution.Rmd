...
#  library(tidyverse)    # Load libraries you need to use
...
$(2)$存在随机向量$\textbf{Z}=[Z_1,\dots,Z_M]^T$（它的每个元素服从独立标准正态分布）， 向量$\bm{\mu}=[\mu_1, \dots, \mu_N]^T$ 及 $N \times M$矩阵$A$满足$\textbf{X}=A\textbf{Z}+\bm{\mu}$。     

$(3)$存在$\bm{\mu}$和一个对称半正定阵$\bm{\Sigma}$满足$\textbf{X}$的特征函数：
$$\varphi_X(\bm{\mu};\bm{\mu},\bm{\Sigma})=\rm{exp}(i\bm{\mu}^T\bm{\mu}-\frac{1}{2}\bm{\mu}^T\bm{\Sigma}\bm{\mu})$$   
如果一个$p$维随机向量$\textbf{X}$服从**多元正态分布（多元高斯分布）**，其联合概率密度函数为
$$f(\textbf{x})=\frac{1}{(2\pi)^{p/2}|\bm{\Sigma}|^{1/2}}exp\{-\frac{1}{2}(\textbf{x}-\bm{\mu})^T \bm{\Sigma}^{-1}(\textbf{x}-\bm{\mu})\},(\bm{\Sigma}>0)$$
其中$\bm{\mu}$是均值向量，$\bm{\Sigma}$是对称正半定矩阵，记为$\textbf{X} \sim N_p(\bm{\mu},\bm{\Sigma})$。    

其数学期望和方差分别为  
$$E(\textbf{X})=\bm{\mu}, D(\textbf{X})=\bm{\Sigma}.$$
```

## 威希特分布（Wishart分布）

```{definition}
假设$\textbf{X}_{(\alpha)}(\alpha=1,2,...,n)$相互独立，且$\textbf{X}_{(\alpha)} \sim N_p(0,\bm{\Sigma})$,则随机$p \times p$矩阵 $\textbf{W} = \sum_{i=1}^{p} x_i x_i^T$ 服从自由度为$n$的**威希特分布**，记为$\textbf{W} \sim W_p(n,\bm{\Sigma})$。其概率密度函数为
$$f(\textbf{W})=\frac{1}{2^{\frac{np}{2}}|\bm{\Sigma}|^{\frac{n}{2}}\bm{\Gamma}_p(\frac{n}{2})}|\textbf{W}|^{\frac{n-p-1}{2}}\rm{exp}\{-\frac{1}{2}\rm{tr}(\bm{\Sigma}^{-1}\textbf{W})\}$$   
  
其数学期望和方差分别为  
$$E(\textbf{W})=n\bm{\Sigma},$$
$$D(w_{ij})=n(\sigma_{ij}^2+\sigma_{ii}\sigma_{jj})$$
对于$n \ge p$的矩阵$\textbf{W}$,如果$\bm{\Sigma}$是可逆的,那么它也是可逆的。当$p=\bm{\Sigma}=1$，那么这个分布是一个自由度为$n$的$\chi^2$分布。
```

## $T^2$分布(Hotelling $T^2$ 分布)

```{definition}
假设$\textbf{W} \sim W_p(n,\bm{\Sigma})$，$\textbf{X} \sim N_p(0,\bm{\Sigma})$，$n \ge p$，$\bm{\Sigma}>0$, $\textbf{W}$与$\textbf{X}$相互独立,则随机变量
...
```{definition}
假设$\textbf{W}_1 \sim \textbf{W}_p(n_1,\bm{\Sigma})$, $\textbf{W}_2 \sim \textbf{W}_p(n_2,\bm{\Sigma})$,  $\bm{\Sigma}>0$, $n_1>p$,$\textbf{W}_1$与$\textbf{W}_2$相互独立, 则
